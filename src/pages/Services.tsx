      ...
      <div className="grid md:grid-cols-3 gap-6 mb-16">
        {tiers.map((tier, index) => (
          <Reveal key={tier.name} delay={index * 100}>
            <div
              className={`rounded-2xl p-8 border backdrop-blur-sm flex flex-col h-full hover:-translate-y-1 hover:shadow-lg transition-all duration-300 ${
                tier.highlight
                  ? 'border-pink-400/50 bg-gradient-to-b from-pink-500/10 to-blue-500/10 hover:shadow-pink-500/20'
                  : 'border-white/10 bg-white/5 hover:border-pink-400/30 hover:shadow-pink-500/10'
              }`}
            >
              ...
              <Link
                to="/contact"
                className={`text-center px-5 py-3 rounded-full font-sans normal-case font-medium transition-all hover:opacity-90 hover:scale-105 active:scale-95 ${
                  tier.highlight
      ...
      <div className="border-t border-white/10 pt-10">
        ...
        <div className="grid md:grid-cols-3 gap-6 font-sans normal-case">
          {extras.map((extra) => (
            <div
              key={extra.name}
              className="flex items-center justify-between border border-white/10 rounded-xl px-5 py-4 text-sm text-slate-300 hover:border-pink-400/30 transition-colors"
            >
